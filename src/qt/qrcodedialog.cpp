...
{
    QString ret = QString("unbreakablecoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non UNB unit input in lnReqAmount, we generate the URI with UNB as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::UNB, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
